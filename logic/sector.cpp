...
GnfrSector::GnfrSector(std::string_view name, GnfrId id, std::string_view description, EmissionDestination destination)
: _id(id)
, _name(name)
...
NfrSector::NfrSector(std::string_view name, NfrId id, GnfrSector gnfr, std::string_view description)
: _id(id)
, _gnfr(gnfr)
, _name(name)
, _description(description)
...
SectorInventory::SectorInventory(std::vector<GnfrSector> gnfrSectors, std::vector<NfrSector> nfrSectors)
...
{
    if (auto iter = find_sector(str, _gnfrSectors); iter != _gnfrSectors.end()) {
        return *iter;
    }
...
{
    if (auto iter = find_sector(str, _nfrSectors); iter != _nfrSectors.end()) {
        return *iter;
    }
